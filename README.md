...
To use the Library Manager API, you can perform HTTP requests using Swagger. I've included a list of JSON objects below for testing.
GUIDs will auto generate so i've left them out of the templates. When performing PUT operations be sure to grab the correct GUIDs from
Swagger using GET (or librarymanager.db file if using VisualStudioCode) and add to the JSON payload. Do not use GUIDs generated by Swagger.
